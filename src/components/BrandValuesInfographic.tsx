...
import Chandan from "../assets/team/mypic.png";
import valuesInfographic from "./BrandValuesInfographic";
...
const values = [
	...
	},
	{
		...
		icon: (
			<svg
				className="w-10 h-10 text-[#4682B4]"
				fill="none"
				stroke="currentColor"
				strokeWidth="2"
				viewBox="0 0 24 24"
...
const BrandValuesInfographic: React.FC = () => (
	<section className="bg-[#F8F9FA] py-12 px-4">
		<div className="max-w-5xl mx-auto">
			<h2 className="text-3xl md:text-4xl font-bold text-center text-[#001F3F] mb-10">
				Our Core Values
			</h2>
			<div className="flex flex-col items-center mb-10">
				<div className="w-24 h-24 rounded-full bg-white shadow-lg flex items-center justify-center border-4 border-[#001F3F] mb-8">
					...
					>
						...
						/>
						<path
							d="M18 36c2-4 6-8 10-8s8 4 10 8"
							stroke="#4682B4"
							strokeWidth="3"
							strokeLinecap="round"
						/>
				...
				{values.map((value, idx) => (
					...
					>
						<div
							...
							style={{
								background:
									idx % 2 === 0 ? "#4682B4" : "#001F3F",
								opacity: 0.12,
							}}
						></div>
						{value.icon}
						<h3 className="text-lg font-semibold text-[#001F3F] mb-2 z-10">
							{value.name}
						</h3>
						<p className="text-gray-700 text-sm mb-0 z-10">
							{value.desc}
						</p>
					</div>
				))}
